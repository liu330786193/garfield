...
public class ContextManager implements TracingContextListener, BootService, IgnoreTracerContextListener {
        ...
        if (context == null) {
            if (StringUtil.isEmpty(operationName)) {
                if (logger.isDebugEnable()) {
                    logger.debug("No operation name, ignore this datacarrier.");
                }
                context = new IgnoredTracerContext();
            } else {
                if (Config.Agent.APPLICATION_ID != DictionaryUtil.emptyValue()
                    && NetworkSingleton.getInstance().getInstanceId() != DictionaryUtil.emptyValue()
                    ) {
                    ...
                    /**
                     * Can't register to collector, no need to datacarrier anything.
                     */
                    context = new IgnoredTracerContext();
                }
            }
    ...
    /**
     * @return the first global datacarrier id if needEnhance. Otherwise, "N/A".
     */
